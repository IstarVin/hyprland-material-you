...
import { settings_file, theme, theme_settings } from "variables.ts";
...
const color_generator =
  `${GLib.get_home_dir()}/dotfiles/material-colors/generate.py`;
...
function hueToHex(hue: number): string {
  ...
  let hexColorStr = `#${toHex(rgb[0] * 255)}${toHex(rgb[1] * 255)}${
    toHex(rgb[2] * 255)
  }`;
        ...
        Widget.Switch({
          ...
          on_activate: (self) => {
            if (self.active != (theme.value.trim() == "dark")) {
              Utils.execAsync(
                `${App.configDir}/scripts/dark-theme.sh --set ${
                  self.state ? "dark" : "light"
                }`,
              ).catch(print);
        ...
        Widget.Entry({
          ...
          on_change: (self) => {
            if (self.text!.length > 6) {
              ...
              self.css = `border: 2px solid; border-color: ${
                hueToHex(Number(self.text))
              }`;
