...
const WifiNetwork = (access_point: AccessPoint) => {
  ...
  const button = Widget.Button({
    ...
    on_secondary_click_release: () => {
      Utils.execAsync(
        `${App.configDir}/scripts/network.sh --edit ${access_point.ssid}`,
      ).catch(print);
    },
    attribute: {
...
const WifiList = () => {
  const updateNetwork = (
    ...
    accessPoints.forEach((accessPoint) => {
      const existing_network = self.children.find((child) =>
        child.attribute.ssid === accessPoint.ssid
      );
  ...
  Widget.Box({
    ...
    setup: (self) => {
      self.hook(systemtray, () => {
        const nm_applet = systemtray.items.find((item) =>
          item.id == "nm-applet"
        );
