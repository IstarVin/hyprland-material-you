    ...
    child: Widget.Box({
      children: [
        Widget.Box({
          ...
          setup(self) {
            const cache_file =
              `${GLib.get_home_dir()}/.cache/ags/thumbnails/wallpaper/${name}`;
...
function CacheThumbnails() {
  Utils.execAsync(
    `mkdir -p ${GLib.get_home_dir()}/.cache/ags/thumbnails/wallpaper`,
  ).catch(print);
  ...
  fileList.forEach((value, index) => {
    ...
    const cache_file =
      `${GLib.get_home_dir()}/.cache/ags/thumbnails/wallpaper/${value}`;
    Utils.idle(() => {
      Utils.readFileAsync(cache_file).catch(() => {
        Utils.execAsync(
          `magick ${path} -resize x32 -gravity Center -extent 1:1 ${cache_file}`,
        ).catch(print);
      });
    });
  });
        ...
        children: [
          Widget.Box({
            ...
            children: [
              Widget.Label({
                class_name: "description",
                label: changing_state.bind().as((state) =>
                  changing_states[state]
                ),
              }),
            ],
          }),
