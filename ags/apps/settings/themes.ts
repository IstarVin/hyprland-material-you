...
const get_themes_cmd =
  `python -OOO ${App.configDir}/scripts/themes.py -a ${themes_folder}`;
let _themes = Utils.exec(get_themes_cmd);
const themes: VariableType<ThemeJson[]> = Variable(JSON.parse(_themes));
const focused: VariableType<ThemeJson | null> = Variable<ThemeJson | null>(
  null,
);
...
const MESSAGES = {
  may_not_be_compatible:
    "This theme may not be compatible with the current version of hyprland-material-you.",
  unknown_version:
    "This theme does not specify ConfigVersion, the theme may not be compatible.",
};
          ...
          children: [
            theme.config_version == "universal" ||
              (theme.config_version != "0" &&
                theme.config_version == config_version)
              ? null
              : MaterialIcon("error", "18px", {
                tooltip_text: theme.config_version == "0"
...
const theme_list = () => {
  ...
  const box = Widget.Box({
    ...
    children: [
      _default,
      ...themes.value.filter((value) => !value.hide).map((value) =>
        theme(value)
      ),
    ],
    vexpand: true,
