...
const range = (length: number, start = 1) =>
  Array.from({ length }, (_, i) => i + start);
function forMonitors(
  widget: (index: number) => Window<any, any>,
): Window<any, any>[] {
  const n = Gdk.Display.get_default()?.get_n_monitors() || 1;
  return range(n, 0).map(widget).flat(1);
}
function forMonitorsAsync(
  widget: (index: number) => Promise<Window<any, any>>,
) {
  const n = Gdk.Display.get_default()?.get_n_monitors() || 1;
...
function load_custom_css() {
  if (configuration.config.current_theme == null) reload_css();
    ...
    const theme_info: ThemeJson = JSON.parse(
      Utils.exec(
        `python -OOO ${App.configDir}/scripts/themes.py -f ${configuration.config.current_theme}`,
      ),
    );
  ...
  } else if (custom_theme.value?.path != configuration.config.current_theme) {
    reload_css();
  }
}
...
Utils.monitorFile(
  `${GLib.get_home_dir()}/.cache/material/colors.css`,
  reload_css,
);
