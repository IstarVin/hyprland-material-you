...
function ClipHistItem(entry: string) {
    ...
    let button = Widget.Button({
        ...
        onSecondaryClick: () => {
            ...
            Utils.execAsync(
                `${App.configDir}/scripts/cliphist.sh --delete-by-id ${id}`,
            );
        },
    });
    ...
    button.connect("clicked", () => {
        ...
        if (clickCount === 1) {
            App.closeWindow(WINDOW_NAME);
            Utils.execAsync(
                `${App.configDir}/scripts/cliphist.sh --copy-by-id ${id}`,
            );
            clickCount = 0;
        }
    });
    if (matches) {
        ...
        if (format == "png") {
            ...
            button.connect("clicked", () => {
                if (!_show_image) {
                    Utils.execAsync(
                        `${App.configDir}/scripts/cliphist.sh --save-by-id ${id}`,
                    )
                        .then((file) => {
...
function ClipHistWidget({ width = 500, height = 500, spacing = 12 }) {
    ...
    async function repopulate() {
        output = await Utils.execAsync(
            `${App.configDir}/scripts/cliphist.sh --get`,
        )
            .then((str) => str)
            .catch((err) => {
                print(err);
        ...
        const list_ids = new Set(
            list.children.map((item) => item.attribute.id),
        );
    ...
    const entry = Widget.Entry({
        ...
        on_change: ({ text }) => {
            ...
            list.children.forEach((item) => {
                item.visible = item.attribute.content.toLowerCase().includes(
                    searchText,
                );
            });
        },
