...
function NotificationIcon(
    { app_entry, app_icon, image }: NotificationType,
): Box<GtkType.Widget, any> {
    if (image) {
        return Widget.Box({
        ...
        children: [
            Widget.EventBox({
                on_primary_click: (box) => {
                    // @ts-expect-error
                    const label: Label<any> = box.child.children[1]!
                        .children[1]!;
                    if (label.lines < 0) {
                        label.lines = 3;
                        label.truncate = "end";
...
export function NotificationPopups(
    ...
    function onNotified(_: any, id: number) {
        ...
        if (
            exclude.some((excludedApp) => appName.includes(excludedApp.toLowerCase()))
        ) return;
        if (
            !include.some((includedApp) => appName.includes(includedApp.toLowerCase())) && include.length > 0
        ) return;
