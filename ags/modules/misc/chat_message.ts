...
`;
    ...
    const wholeThing = Box({
        ...
        attribute: {
            updateText: (text) => {
                latexViewArea.attribute.render(latexViewArea, text).catch(
                    print,
                );
            },
        },
            ...
            fullUpdate: (self, content: string, useCursor = false) => {
                ...
                for (const [index, line] of lines.entries()) {
                    ...
                    if (codeBlockRegex.test(line)) {
                        ...
                        const blockContent = lines.slice(lastProcessed, index)
                            .join("\n");
                        if (!inCode) {
                ...
                if (lastProcessed < lines.length) {
                    const kids = self.get_children();
                    const lastLabel = kids[kids.length - 1];
                    let blockContent = lines.slice(lastProcessed, lines.length)
                        .join(
                            "\n",
                        );
                    if (!inCode) {
...
export const ChatMessage = (message, modelName = "Model") => {
    ...
    const messageLoadingSkeleton = Box({
        ...
        children: Array.from(
            { length: 3 },
            (_, id) => TextSkeleton(`skeleton_line_offset${id}`),
        ),
    });
