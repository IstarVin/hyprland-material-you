...
const Audio = () => {
            ...
            self.hook(cur_page, () => {
                self.children.forEach((widget) => {
                    widget.toggleClassName(
                        "active",
                        widget.attribute.page == cur_page.value,
                    );
                });
            });
        },
    ...
    const speakers = Widget.Box({
        vertical: true,
        children: [] as ReturnType<typeof Speaker>[],
    });
    const apps = Widget.Box({
        vertical: true,
        children: [] as ReturnType<typeof app>[],
    });
    ...
    function update() {
        ...
        audio.speakers.forEach((s) => {
            if (!speakers.children.some((v) => v.attribute.stream.id == s.id)) {
                speakers.pack_start(
                    Speaker(s, icons.speaker, off_icons.speaker),
                    false,
                    false,
                    0,
                );
    ...
    update();
    ["notify::speakers", "notify::apps", "notify::microphones"].forEach(
        (event) => {
            audio.connect(event, () => {
