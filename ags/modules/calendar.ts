...
const calendar_day = (day: string, today: number) =>
  Widget.Button({
    class_name: `calendar_button ${
      today == 1 ? "today" : today == -1 ? "othermonth" : ""
    }`,
...
const calendar_widget = () => {
  ...
  function shift_x_months(x: number): void {
    ...
    const new_date: Date = month_shift == 0
      ? new Date()
      : get_date_in_x_months_time(month_shift);

    calendar_json = get_calendar_layout(new_date, month_shift == 0);
    month_year.label = `${month_shift == 0 ? "" : "• "}${
        ...
        Widget.Box({
          ...
          children: [
            ...
            Widget.Box({
              homogeneous: true,
              spacing: 5,
              children: week_days.map((day, i) =>
                calendar_day(day.day, day.today)
              ),
            }),
            calendar_days,
          ],
