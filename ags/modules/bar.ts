...
function getIconNameFromClass(windowClass: string) {
  ...
  let systemDataDirs = GLib.get_system_data_dirs().map((dir) =>
    dir + "/applications"
  );
...
const dispatch = (ws: string) =>
  hyprland.messageAsync(`dispatch workspace ${ws}`).catch(print);
...
function Workspaces() {
  ...
  >();
  const workspace_buttons_array: VariableType<Button<any, any>[] | any> =
    Variable([]);
  ...
  function update() {
    workspace_buttons.forEach((workspace) => {
      const existingWorkspace = hyprland.workspaces.some((element) =>
        element.id === workspace.attribute.id
      );
  ...
  return Widget.Box({
    ...
    children: [
      ...
      Widget.Label({
        ...
        visible: config.bind("config").as((config) =>
          config.show_battery_percent
        ),
      }),
      Widget.Label({
        label: battery.bind("energy_rate").as((energy_rate) =>
          `  ${energy_rate.toFixed(2)}W`
        ),
        visible: battery.bind("energy_rate").as((energy_rate) =>
          energy_rate != 0
        ),
      }),
    ],
...
function Network() {
  return Widget.Button({
    ...
    if (network.wired.internet === "connected") {
      ...
      self.child.label =
        MATERIAL_SYMBOL_SIGNAL_STRENGTH[network.wifi.icon_name] ||
        "signal_wifi_off";
    } else {
  ...
  const button = Widget.Button({
    ...
    .hook(mpris, (self) => {
      if (mpris.players.length > 0) {
        ...
        if (metadata) {
          self.tooltip_text = `${metadata["xesam:artist"]} - ${
...
function TaskBar() {
  ...
  function Clients(clients: Client[]) {
    ...
    globalWidgets = globalWidgets.filter((widget) =>
      currentClientIds.includes(widget.attribute.client.address)
    );
    ...
    clients.forEach((client) => {
      ...
      let widget = globalWidgets.find((w) =>
        w.attribute.client.address === client.address
      );
      if (widget) {
...
function volumeIndicator() {
  return Widget.EventBox({
    ...
    child: Widget.Button({
      ...
      on_secondary_click_release: () =>
        Utils.execAsync("pavucontrol -t 3").catch(print),
...
function Left() {
  const workspaces = config.config.workspaces_to_the_left
    ? Workspaces()
    : undefined;
