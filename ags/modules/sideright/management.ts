            ...
            Widget.Box({
                ...
                children: [
                    Widget.Button({
                        ...
                        on_clicked: () => {
                            Utils.execAsync(
                                `${App.configDir}/scripts/dark-theme.sh --toggle`,
                            )
                                .catch(print);
                            theme.setValue(
                                theme.value.trim() == "dark" ? "light" : "dark",
                            );
                        },
                        child: IconAndName({
                            label: "Dark theme",
                ...
                children: [
                    Widget.Button({
                        ...
                        on_clicked: () => {
                            ...
                            } else {Utils.execAsync(
                                    "pkill -f wayland-idle-inhibitor.py",
                                )
                                    .catch(print);}
                        },
                        setup: (self) => {
                            globalThis.toggleIdleInhibitor = () => self.on_clicked(self);
                            idle_inhibitor.setValue(
                    ...
                    Widget.Button({
                        ...
                        on_clicked: () => {
                            ...
                            Utils.execAsync(
                                `${App.configDir}/scripts/night-light.sh --toggle`,
                            )
                                .then((out) => {
                                    night_light.setValue(
                                        out.trim() == "enabled",
                                    );
                                })
                                .catch((err) => {
...
// globalThis.toggleIdleInhibitor = () => {
//     idle_inhibitor.value = !idle_inhibitor.value
// }
                    ...
                    Widget.Button({
                        ...
                        on_clicked: () => {
                            ...
                            Utils.execAsync("sleep 0.5")
                                .then(() =>
                                    Utils.execAsync("hyprpicker -a").catch(
                                        print,
                                    )
                                )
                                .catch(print);
...
export function Management() {
    ...
    return Widget.EventBox({
        on_scroll_down: () =>
            current_page.setValue(
                Math.min(current_page.value + 1, number_of_pages - 1),
            ),
        ...
        child: Widget.Box({
            ...
            setup: (self) => {
                for (let page in page_names) {
                    // @ts-expect-error
                    self.keybind(
                        `${Number(page.replace("page", "")) + 1}`,
                        () => {
                            current_page.setValue(
                                Number(page.replace("page", "")),
                            );
                        },
                    );
                }
