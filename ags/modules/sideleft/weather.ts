...
const reload = () => {
  if (
    configuration.config.weather != "" &&
    configuration.config.weather_location_id != ""
  ) {
    Utils.execAsync(
...
function WeatherFirstCard(weather: WeatherJson) {
  ...
  const description = Widget.Label({
    ...
    vpack: "center",
    hpack: "start",
    label: weather.weather[0].description.charAt(0).toUpperCase() +
  ...
  else if (_visibility >= 1000) {
    visibility = `${(_visibility / 1000).toFixed(2)}km`;
