...
if (!fileExists(`${GLib.get_user_config_dir()}/gemini_history.json`)) {
  ...
  Utils.writeFile("[ ]", `${GLib.get_user_config_dir()}/gemini_history.json`)
    .catch(print);
}

Utils.exec(`mkdir -p ${GLib.get_user_state_dir()}/ags/user/ai`);
const KEY_FILE_LOCATION =
  `${GLib.get_user_state_dir()}/ags/user/ai/google_key.txt`;
const APIDOM_FILE_LOCATION =
  `${GLib.get_user_state_dir()}/ags/user/ai/google_api_dom.txt`;
...
class GeminiMessage extends Service {
  ...
  parseSection() {
    ...
    if (!parsedData.candidates) {
      this._parts[0].text +=
        `Blocked: ${parsedData.promptFeedback.blockReason}`;
    } else {
