...
class GeminiService extends Service {
    ...
    set cycleModels(value) {
        ...
        if (!value) this._modelIndex = 0;
            ...
            this._modelIndex = (this._requestCount - (this._requestCount % ONE_CYCLE_COUNT)) %
                CHAT_MODELS.length;
        }
    }
    ...
    saveHistory() {
        Utils.exec(
            `bash -c 'mkdir -p ${HISTORY_DIR} && touch ${HISTORY_PATH}'`,
        );
        Utils.writeFile(
            JSON.stringify(
    ...
    send(msg) {
        ...
        if (this._cycleModels) {
            this._requestCount++;
            if (this._cycleModels) {
                this._modelIndex = (this._requestCount -
                    (this._requestCount % ONE_CYCLE_COUNT)) %
                    CHAT_MODELS.length;
            }
        }
    }
